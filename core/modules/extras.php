<?php
/* extra functions
dw this wont be in the release or beta version
and libraries will be modules that i use, i.e delight-im/PHP-Cookie and cakephp/validation
*/

// htmlspecialchars shorterned
function nxss(string|null $string = "", int $flags = ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401, ?string $encoding = null, bool $double_encode = true): string {
...
function response(array|null $array, bool $success, string|int $name, string|int $type, string|int $value) {
    if(isset($array)) {
        if($success) {
            return [
                "success" => true,
                "message" => "feed message sent"
            ];
        } else {
            return [
                "success" => false,
                "errors" => [
                    "feed" => [ "_length" => "your message is ".abs(strlen($_GET["message"]) - 300)." character(s) too long"
                    ],
                ],
            ];
        }
